...
public class LinqEnhancementsDemo : IAsyncDemo
{
    private readonly InventoryDbContext _db;

    public LinqEnhancementsDemo(InventoryDbContext db)
    {
        _db = db;
    }
    ...
    public async Task RunAsync()
    {
        ...
        //-------------------------------------------------
        //TODO: Put the code from Listing 14-24 here:
        var itemsByGenres = new List<Item>();

        //-------------------------------------------------
        if (itemsByGenres.Count() > 0)
        { 
            Console.WriteLine(ConsolePrinter.PrintBoxedList(itemsByGenres
                , i => $"{i.ItemName} - Genres: {string.Join(", ", i.Genres.Select(g => g.GenreName))}"));
        }
    }
